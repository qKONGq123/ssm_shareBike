...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">归还记录管理</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-two"><span>归还记录列表</span></li>
			</ol>
		</div>
		<!-- /Breadcrumb -->
		...
		<div id="center-container">
			...
			<div class="center-form">
				...
				<div class="form-item">
					<label>车辆评分</label>
					<select name="cheliangpingfen" id="cheliangpingfenSelect" class="form-control form-control-sm" aria-controls="tableId">
						<option value=" "></option>
						<option value="★">★</option>
						<option value="★★">★★</option>
						<option value="★★★">★★★</option>
			...
			<table id="tableId" class="table table-bordered">
				<thead>
					<tr>
						...
						<th onclick="sort('cheliangpingfen')">车辆评分<i id="cheliangpingfenIcon" class="fa fa-sort"></i></th>
						<th>车辆评价</th>
	...
	<script>
		...
		function search(){
			...
			var cheliangpingfenIndex = document.getElementById("cheliangpingfenSelect").selectedIndex;
			var cheliangpingfenOptions = document.getElementById("cheliangpingfenSelect").options;
			var cheliangpingfenSelectedOption = cheliangpingfenOptions[cheliangpingfenIndex]
			if(cheliangpingfenSelectedOption.text != null && cheliangpingfenSelectedOption.text != ''){
				searchForm.cheliangpingfen = cheliangpingfenSelectedOption.text;
			}
		...
		function setDataRow(item,number){
			...
			var cheliangpingfenCell  = document.createElement('td');

						 cheliangpingfenCell.innerHTML = item.cheliangpingfen;

			row.appendChild(cheliangpingfenCell);
			var cheliangpingjiaCell  = document.createElement('td');

						  if(item.cheliangpingjia != null && item.cheliangpingjia != "" && item.cheliangpingjia.length >= 11){
			  	cheliangpingjiaCell.innerHTML = item.cheliangpingjia.substring(0, 10) + "..."; 
			  }else{
			  	cheliangpingjiaCell.innerHTML = item.cheliangpingjia;
			  }


			row.appendChild(cheliangpingjiaCell);
		...
		function sh(){
			...
			httpJson("guihaijilu/update","POST",data,(res)=>{
				if(res.code == 0){
					alert("提交成功");
					getDataList();
				}
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("guihaijilu/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"guihaijilu");
        ...
        function pay(item){
			...
			window.sessionStorage.setItem('tableName',"guihaijilu");
